...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        ##################################################
        # Replace "pass" statement with your code
        # Gradient of the loss with respect to the input x without dropout
...
class Linear_ReLU(object):
    ...
    def forward(x, w, b):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        linear_result = torch.mm(x.view(x.shape[0], -1), w) + b
        linear_cache = (x, w, b, linear_result)
        out =  torch.max(torch.zeros_like(linear_result), linear_result)
        relu_cache = (linear_result, out)
    ...
    def backward(dout, cache):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        ### using chain rule for EBP 
        linear_cache, relu_cache = cache    
        drelu = dout * (relu_cache[1] > 0).float()
        x, w, b, linear_result = linear_cache
        dx = drelu.mm(w.t()).view(x.shape)
        dw = x.view(x.shape[0], -1).t().mm(drelu)
        db = drelu.sum(dim=0)
        ######################################################################
        #                END OF YOUR CODE                                    #
        ######################################################################
...
def softmax_loss(x, y):
    ...
    loss = -torch.log(probs[range(num_samples), y]).mean()
    dx = probs.clone()
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ###################################################################
        self.params['W1'] = torch.normal(mean= 0 , std= weight_scale,  dtype=torch.float64, size=(hidden_dim, input_dim), device = device).t()
        self.params['b1'] = torch.zeros((1,hidden_dim), dtype=torch.float64, device = device)
        self.params['W2'] = torch.normal(mean= 0 , std= weight_scale, dtype=torch.float64, size=( num_classes, hidden_dim), device = device).t()
        self.params['b2'] = torch.zeros((1,num_classes), dtype=torch.float64, device = device)
    ...
    def loss(self, X, y=None):
        ...
        # scores variable.                                          #
        #############################################################
        out = torch.mm(X, self.params['W1']) + self.params['b1']
        out1 = torch.max(torch.zeros_like(out), out)
        scores = torch.mm(out1, self.params['W2']) + self.params['b2']
        ...
        exp_x = torch.exp(scores)
        probs = exp_x / exp_x.sum(dim=1, keepdim=True)
        indices = torch.arange(X.shape[0])
        log_probs = torch.log(probs[indices, y])
        train_loss = - log_probs.mean()
        l2_regularization = self.reg * torch.sum(self.params['W1'] ** 2) + self.reg * torch.sum(self.params['W2'] ** 2)
        loss = train_loss + l2_regularization
        
        grads_scores = probs.clone()
        grads_scores[indices, y] -= 1
        grads_scores /= scores.shape[0]
        grads['W2'] = torch.mm(out1.t(), grads_scores) + 2 * self.reg * self.params['W2']  
        grads['b2'] = torch.sum(grads_scores, dim=0)

        grads_z1 = torch.mm(grads_scores, self.params['W2'].t())
        grads_z1[out1 <= 0] = 0 

        grads['W1'] = torch.mm(X.t(), grads_z1) + 2 * self.reg * self.params['W1'] 
        grads['b1'] = torch.sum(grads_z1, dim=0)
        return loss, grads
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for hidden_dim in hidden_dims:
            if k == 1 : 
                self.params[f'W{k}'] = torch.normal(mean= 0 , std= weight_scale, dtype=torch.float64, size=(hidden_dim, input_dim), device = device).t()
                self.params[f'b{k}'] = torch.zeros((1,hidden_dim), dtype=torch.float64, device = device)
            else:
                self.params[f'W{k}'] = torch.normal(mean= 0 , std= weight_scale, dtype=torch.float64, size=(hidden_dim, hidden_dims[k - 2]), device = device).t()
                self.params[f'b{k}'] = torch.zeros((1, hidden_dim), dtype=torch.float64, device = device)
            k = k + 1 
        self.params[f'W{k}'] = torch.normal(mean= 0 , std= weight_scale, dtype=torch.float64, size=(num_classes, hidden_dims[-1]), device = device).t()
        self.params[f'b{k}'] = torch.zeros((1, num_classes), dtype=torch.float64, device = device)
    ...
    def loss(self, X, y=None):
        ...
        cache = []
        cached = []
        for m in range(1, self.num_layers + 1 ): 
            if m == self.num_layers : 
                scores , c = Linear.forward(out[-1], self.params[f'W{m}'], self.params[f'b{m}'])
                cache.append(c)
            else: 
                o , c = Linear_ReLU.forward(out[-1], self.params[f'W{m}'], self.params[f'b{m}']) 
                if self.use_dropout:
                    o, cd = Dropout.forward(o, self.dropout_param)
                    cached.append(cd)
                out.append(o)
                cache.append(c)
        ...
        l2_regularization = 0
        for m in range(1, self.num_layers + 1): 
            l2_regularization +=  (0.5 * self.reg * torch.sum(self.params[f'W{m}'] ** 2))
        loss = train_loss + l2_regularization
        ...
        grads_scores = probs.clone()
        grads_scores[indices, y] -= 1
        grads_scores /= batch_size

        for i in range(self.num_layers, 0, -1):
            if i == self.num_layers:
                dx, grads[f'W{i}'], grads[f'b{i}'] = Linear.backward(grads_scores, (cache[i-1]))
            else:
                if self.use_dropout:
                    dx = Dropout.backward(dx, cached[i-1])
                dx, grads[f'W{i}'], grads[f'b{i}'] = Linear_ReLU.backward(dx, cache[i-1])
...
def create_solver_instance(data_dict, dtype, device):
    model = TwoLayerNet(hidden_dim=200, dtype=dtype,weight_scale=5e-2, device=device)
    #############################################################
    # TODO: Use a Solver instance to train a TwoLayerNet that   #
    # achieves at least 50% accuracy on the validation set.     #
    #############################################################
    # , lr_decay= 0.1, batch_size = 2000, num_epochs =  40 , num_train_samples =  200000, num_val_samples =   None
    solver = Solver(model, data_dict, update_rule = sgd, lr_decay= 1, batch_size = 32, num_epochs =  20, device=device)
...
def sgd(w, dw, config=None):
    ...
    if config is None:
        config = {}
    config.setdefault('learning_rate', 9e-3)
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            mask = (torch.rand_like(x) > p) / (1 - p)  # Generate dropout mask
            out = x * mask  # Apply dropout mask during training
